...
public class CommentTokenMapper {
    public record CommentWithTokenInfo(BagOfWords bag, int startIndex, int length, Submission submission) {
    }
    ...
    private void mergeCommentTokens(Submission submission) {
        ...
        List<String> currentCommentTokens = new ArrayList<>();
        int lastStart = -1;
        for (int index = 0; index < tokens.size(); index++) {
            Token token = tokens.get(index);
            if (token.getType() == SharedTokenType.FILE_END || token.getType() == CommentTokenType.COMMENT_END) {
                if (currentCommentTokens.size() > 0 && index > lastStart) {
                    commentsOfThisSubmission
                            .add(new CommentWithTokenInfo(new BagOfWords(currentCommentTokens), lastStart, index - lastStart, submission));
                }
                lastStart = -1;
                ...
                currentCommentTokens.add(token.getType().getDescription());
            }
        }
